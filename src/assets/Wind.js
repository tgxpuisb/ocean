...
var WindyLayer = function (_ol$layer$Image) {
  ...
  function WindyLayer(data) {
    ...
    var imagecanvas = require('ol/source/imagecanvas').default
    _this.setSource(new imagecanvas({
      logo: options.logo,
      state: options.state,
  ...
  WindyLayer.prototype.appendTo = function appendTo(map) {
    let mapClass = require('ol/map').default
    if (map && map instanceof mapClass) {
  ...
  return WindyLayer;
}(require('ol/layer/image').default);
